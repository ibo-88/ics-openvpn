-- ThrowingSpade.lua
-- Модель метательной лопаты
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local ModelBuilder = require(ReplicatedStorage.Assets.ModelBuilder)

local function CreateThrowingSpade()
    local model = Instance.new("Model")
    model.Name = "ThrowingSpade"
    ...
    blade.Name = "Blade"
    blade.Size = Vector3.new(0.25, 0.35, 0.08)
    blade.Position = Vector3.new(0, 1.25, 0)
    blade.Anchored = true
    ...
    tip.Position = Vector3.new(0, 1.43, 0)
    ...
    -- Атрибуты
    model:SetAttribute("WeaponType", "throwing_spade")
    ...
    model:SetAttribute("WeaponWeight", 2.2)
    ...
    model:SetAttribute("WeaponValue", 250)
    model:SetAttribute("WeaponDescription", "Метательная лопата с острым лезвием")
    
    return model
end

return CreateThrowingSpade