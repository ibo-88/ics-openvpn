-- ThrowingSword.lua
-- Модель метательного меча
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local ModelBuilder = require(ReplicatedStorage.Assets.ModelBuilder)

local function CreateThrowingSword()
    local model = Instance.new("Model")
    model.Name = "ThrowingSword"
    
    -- Лезвие
    local blade = Instance.new("Part")
    blade.Name = "Blade"
    blade.Size = Vector3.new(0.1, 1.8, 0.08)
    ...
    tip.Size = Vector3.new(0.1, 0.4, 0.08)
    ...
    guard.Name = "Guard"
    guard.Size = Vector3.new(0.4, 0.1, 0.15)
    ...
    fuller.Name = "Fuller"
    fuller.Size = Vector3.new(0.02, 1.5, 0.02)
    fuller.Position = Vector3.new(0, 0.9, 0)
    ...
    decoration1.Position = Vector3.new(0, 0.5, 0)
    ...
    model:SetAttribute("WeaponValue", 700)
    model:SetAttribute("WeaponDescription", "Метательный меч с острым лезвием")
    
    return model
end

return CreateThrowingSword